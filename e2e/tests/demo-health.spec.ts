...
const COURSE = process.env.E2E_COURSE ?? "ACCY-111";
...
test("fire an event and audit the execution end to end", async ({ page, request }, testInfo) => {
  ...
  // 4c. The issuance + delivery bookends exist (whatever the selected branch).
  ...
  expect(
    actions.some((a) => a === "deliver_to_learncard_wallet" || a === "deliver_to_smartresume"),
    "a delivery step must have run",
  ).toBe(true);
