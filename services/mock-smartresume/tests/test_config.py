...
from mock_smartresume.config import _ENV_FILE, Settings
...
def test_settings_read_from_dotenv_regardless_of_cwd(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.delenv("MOCK_SMARTRESUME_PORT", raising=False)
    ...
    _ENV_FILE.write_text("MOCK_SMARTRESUME_PORT=8931\n")
    try:
        assert Settings().port == 8931
    finally:
        if original is None:
            _ENV_FILE.unlink()
        else:
