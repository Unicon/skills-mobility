"""Minimal prompt-injection screen for learner context (FR-DT-24, ADR-0021).

A lightweight, deterministic scan of the free-text string values in
``learner_context`` for common prompt-injection phrasings before they are placed
...
def screen_for_injection(learner_context: dict[str, Any]) -> list[Finding]:
    """Walk the learner context and return a finding for each free-text value that
    matches an injection pattern. Empty list means nothing suspicious was seen."""
    findings: list[Finding] = []
    _walk(learner_context, "learner_context", findings)
    return findings
