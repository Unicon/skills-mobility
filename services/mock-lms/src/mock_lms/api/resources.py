...
from mock_lms.catalog import ROOT_ACCOUNT_ID, CatalogStore
...
@router.get("/courses/{course_id}/pages/{page_id}")
def get_page(course_id: str, page_id: str, store: StoreDep) -> dict[str, Any]:
    page = store.get_page(course_id, page_id)
    if page is None:
        raise _not_found(f"page {page_id} not found in course {course_id}")
...
def get_assignment(course_id: str, assignment_id: str, store: StoreDep) -> dict[str, Any]:
    assignment = store.get_assignment(assignment_id)
    if assignment is None or assignment.course_id != course_id:
        raise _not_found(f"assignment {assignment_id} not found in course {course_id}")
...
@router.get("/courses/{course_id}/rubrics/{rubric_id}")
def get_rubric(course_id: str, rubric_id: str, store: StoreDep) -> dict[str, Any]:
    rubric = store.get_rubric(rubric_id)
    if rubric is None or rubric.course_id != course_id:
        raise _not_found(f"rubric {rubric_id} not found in course {course_id}")
...
@router.get("/courses/{course_id}/assignments/{assignment_id}/submissions/{user_id}")
def get_assignment_submission(
    course_id: str,
    assignment_id: str,
    user_id: str,
    store: StoreDep,
    include: Annotated[list[str] | None, Query(alias="include[]")] = None,  # noqa: ARG001
) -> dict[str, Any]:
    submission = store.submission(course_id, assignment_id, user_id)
    if submission is None:
        raise _not_found(
            f"submission not found for user {user_id} on assignment {assignment_id}"
        )
...
def get_account_users(
    account_id: str,
    store: StoreDep,
    uuids: Annotated[list[str] | None, Query(alias="uuids[]")] = None,
) -> list[dict[str, Any]]:
    """Resolve account users by Canvas UUID (the skill_mastered uuid → user_id step)."""
    if account_id != ROOT_ACCOUNT_ID:
        raise _not_found(f"account {account_id} not found")
    return [u.model_dump(mode="json") for u in store.users_for_uuids(uuids or [])]
