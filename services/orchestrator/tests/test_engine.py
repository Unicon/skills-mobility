...
def test_llm_backed_decisions_are_labeled_llm(sample_event):
    ...
    plan_source="llm", and the accepted (re-bound) plan is labeled on both the
    ...
    assert gate_decision.plan_source == "llm"
    # Delivery Targets is unconfigured here -> its stub fallback stays labeled.
    assert targets_decision.plan_source == "deterministic_fallback"
    assert plan_decision.plan_source == "llm"
...
def test_unbindable_llm_plan_is_labeled_deterministic_fallback(sample_event):
    ...
    assert plan_decision.plan_source == "deterministic_fallback"
    assert meta.status == "completed"  # the fallback plan still executes


def test_issuer_omitted_selection_is_flagged_on_the_targets_decision(sample_event):
    """A non-empty selection without learncard_issuer violates the §5 premise;
    the fact is stored on the delivery_targets decision (orthogonal to
    plan_source — this selection came from a live seam)."""

    class _SmartResumeOnlyDT:
        def select_targets(self, event_type, source_system, learner_context, ctx):
            return ["smart_resume"]
    ...
    meta = engine.run_workflow(
        ...
        delivery_config_ref="cfg",
        recipient_profile_id="smi-demo-learner",
        delivery_targets=_SmartResumeOnlyDT(),
    )
    targets_decision = meta.decisions[1]
    assert targets_decision.kind == "delivery_targets"
    assert targets_decision.plan_source == "llm"
    assert targets_decision.issuer_omitted_from_selection is True
    # And the routine selection elsewhere stays unflagged (see the app tests).
