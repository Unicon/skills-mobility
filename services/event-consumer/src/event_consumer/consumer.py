...
def process(
    ...
    logger.info("ingest received: event_name=%s correlation_id=%s", metadata.get("event_name"),
                correlation_id)

    errors = identity.validate(event)
    if errors:
        store.record_rejection(identity.rejection_key(event), errors)
        logger.info("ingest rejected: correlation_id=%s errors=%s", correlation_id, errors)
    ...
    if existing is not None:
        logger.info(
            "ingest duplicate: correlation_id=%s identity_key=%s existing_execution=%s",
            correlation_id, key, existing,
        )
    ...
    logger.info(
        "ingest created: correlation_id=%s execution_id=%s event_type=%s status=%s",
        correlation_id, execution_id, etype, status,
    )
