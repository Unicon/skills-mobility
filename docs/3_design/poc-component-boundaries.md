...
| **Delivery Router** | Dispatch approved delivery actions to the configured adapter; adapter endpoint and config lookup; shared timeout and retry policy; correlation and execution identifier attachment; normalize adapter responses; record delivery-attempt and delivery-result logs | Target selection; workflow planning; business-policy evaluation; payload reshaping that belongs in the transformation pipeline | Approved delivery action request from Orchestrator; delivery config reference | Normalized delivery result to Orchestrator; delivery-attempt and delivery-result records | LearnCard Issuer Adapter; LearnCard Wallet Adapter; SmartResume Wallet Adapter |
| **LearnCard Profile Resolver** | Resolve a learner identifier to a LearnCard profileId and DID; persist the resolved mapping; create a new profile when none is found; return the resolution method used | Credential issuance; wallet delivery; orchestration planning; delivery routing; payload transformation | Resolution request from Orchestrator; learner identifier | Resolved profileId and DID with resolution method | LearnCard REST API; libs/learncard-api; LearnCard Profile Mapping Store |
| **LearnCard Issuer Adapter** | Issue and sign a credential via the LearnCard SDK; assure the issuer service profile exists before issuance; normalize SDK responses | Payload shaping upstream of adapter contract; recipient profile resolution; wallet delivery; policy decisions | Delivery action request from Delivery Router; pre-shaped unsigned VC with resolved recipient | Signed issued credential | LearnCard SDK |
| **LearnCard Wallet Adapter** | Deliver an already-issued credential to the LearnCard wallet API; acquire a bearer token via libs/learncard-api; normalize delivery API responses | Credential issuance; profile resolution; payload shaping beyond final-mile protocol binding; policy decisions | Delivery action request from Delivery Router; signed credential; resolved recipient profileId | Normalized delivery result | LearnCard REST API; libs/learncard-api |
| **SmartResume Wallet Adapter** | Deliver a credential to SmartResume; handle SmartResume-specific auth and API calls; normalize responses | Target selection; orchestration planning; payload shaping beyond adapter contract | Delivery action request from Delivery Router; shaped credential payload | Normalized delivery result | SmartResume API |
...
| **LearnCard Profile Mapping Store** | Stores resolved learner-identifier-to-profileId and DID mappings so the Profile Resolver can return `stored` results without repeating LearnCard API calls | LearnCard Profile Resolver | LearnCard Profile Resolver |
...
- The **Delivery Router** is a thin delivery execution facade; it dispatches approved actions and applies shared delivery mechanics (config lookup, timeout/retry, correlation, result normalization, delivery logging) but does not reinterpret business logic or reshape payloads. Each named **delivery adapter** (LearnCard Issuer Adapter, LearnCard Wallet Adapter, SmartResume Wallet Adapter) owns only the minimal target-specific conversion needed for its vendor SDK or API; see [ADR-0016](../decisions/0016-delivery-routing-topology.md).
- The **LearnCard Profile Resolver** is a discrete orchestration step invoked directly by the Orchestrator before any LearnCard delivery action. It is not part of the Delivery Router chain; profile resolution runs once upstream and the resolved `profileId`/DID flows into the Orchestrator's execution context for use by the issuer and wallet steps.
