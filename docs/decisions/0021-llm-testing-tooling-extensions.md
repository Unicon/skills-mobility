...
ADR-0013 (Status: Proposed) already established this project's testing framework: a frozen evaluation corpus, per-service scorecards, deterministic validation as the authoritative check for two of the four LLM Decision Services (Workflow Actions, Delivery Targets), a hybrid of deterministic execution and human review when semantic alignment is unclear for a third (Field Mapping, per ADR-0013's scorecard), and LLM-as-judge or targeted human review reserved for the fourth (Field Synthesis, the one genuinely open-ended service). ADR-0013 left several open questions, including how a more automated LLM-as-judge component could be added "without creating too circular of outcomes."
...
This ADR extends ADR-0013 rather than replacing it. It also keeps open a live question: whether the two services currently assumed to be fully deterministic (Workflow Actions, Delivery Targets), plus Field Mapping's deterministic-execution component, might warrant judgment-based evaluation too. This ADR's DeepEval decision is written to keep that door open without conceding the point either way — see "Why This Approach" below.
...
For the upcoming Decision Service docs: DeepEval is a hard dependency for Decision Service implementations (imports, corpus layout), not a contingent choice. Corpus/suite location relative to `libs/`/`services/` remains an explicit open question (see below). CI-based execution is not yet applicable — this POC has no CI pipeline.
