...
A set of tools built around a learner-centric data model, most notably a Metadata Repository (MDR) and Translator that store field-level mappings between education standards and convert learner records between schemas. The MDR has an active bug preventing import of new schemas, blocking integration of the LMS schema needed for the POC. Additionally, the mapping approach (field-by-field rather than payload-level) may not suit an AI-assisted transformation workflow, and LIF itself is in active development with known blockers. Not used in this POC; may be mocked or partially reimplemented.