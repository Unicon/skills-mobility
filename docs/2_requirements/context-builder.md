...
For clarity, some logical identifiers are carried in Canvas-style envelope fields rather than in a body field with the same name. For example, `course_id` may come from `metadata.context_id`, `body.result_context_id`, or `body.course.id` depending on the event shape. In the `skill_mastered` profile, `user_id` is resolved from `metadata.root_account_id` plus `body.user_uuid` via the account-user lookup — this is the canonical Canvas path, since real Canvas `learning_outcome_result_created` events do not carry `user_id` directly. The POC envelope additionally carries `metadata.user_id`; the resolved id should match it, making it a free consistency check. In the `course_completed` profile, the Canvas `user_id` can be taken directly from `metadata.user_id`.
