...
- **Archiving.** When a doc is retired, set `Status: Superseded` (note what replaced it) and move it to an `archive/` subfolder within its phase directory (e.g. `2_requirements/archive/`). ADRs are never moved — they are amended in place as decisions evolve; mark them `Status: Superseded by ADR-XXXX` only when a decision is wholly replaced rather than refined.
