...
This lib has no `.env` of its own. These vars are expected in each **consuming service's** own `.env` (the Profile Resolver and Wallet Adapter) — `LearnCardSettings` reads that file via `env_file=".env"`. Today the values are generated by `tools/learncard-demo` (#52 / PR #54).
